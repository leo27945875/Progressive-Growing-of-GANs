...
class Discriminator(nn.Module):
    def __init__(self, imgChannels, maxChannels, factors):
        ...
        self.initBlock = nn.Sequential(
            BatchStd(),
            HeConv2d(maxChannels + 1, maxChannels),
            nn.LeakyReLU(0.2),
            HeConv2d(maxChannels, maxChannels, 4, 1, 0),
            nn.LeakyReLU(0.2),
            HeConv2d(maxChannels, 1, 1, 1, 0),
            nn.Flatten()
        )
        self.initFromRGB = nn.Sequential(
            HeConv2d(imgChannels, maxChannels, 1, 1, 0)
        )
        ...
        for i in range(len(factors) - 2, -2, -1):
            ...
            self.proBlocks.insert(0, nn.Sequential(
                    ConvBlock(lastChannels, channels, isPixelNorm=False),
                    nn.AvgPool2d(2)
            ))
            self.rgbBlocks.insert(1, nn.Sequential(
                    HeConv2d(imgChannels, lastChannels, 1, 1, 0)
            ))
...
class DiscriminatorLoss(nn.Module):
    def __init__(self, ganLossWeight=1, gpLossWeight=10, driftLossWeight=0.001):
        super().__init__()
        self.w = [ganLossWeight, gpLossWeight, driftLossWeight]
    
    def GetGradientPenalty(self, dis, fakeImgs, realImgs, nBlock, alpha):
        batchSize, c, h, w = fakeImgs.shape
        weights = torch.rand(batchSize, 1, 1, 1).to(fakeImgs.device).repeat(1, c, h, w)
        mixImgs = weights * fakeImgs + (1 - weights) * realImgs
        mixImgs.requires_grad_()
        mixOuts = dis(mixImgs, nBlock, alpha)
        
        grad = torch.autograd.grad(
                inputs=mixImgs,
                outputs=mixOuts,
                grad_outputs=torch.ones_like(mixOuts),
                create_graph=True,
                retain_graph=True
            )[0]
        grad = grad.view(batchSize, -1)
        
        return torch.mean((grad.norm(2, dim=1) - 1) ** 2)
    
    def forward(self, dis, fakeImgs, realImgs, fakeOuts, realOuts, nBlock, alpha):
        ganLoss = fakeOuts.mean() - realOuts.mean()
        gp      = self.GetGradientPenalty(dis, fakeImgs, realImgs, nBlock, alpha)
        drift   = (realOuts ** 2).mean()
        return self.w[0] * ganLoss + self.w[1] * gp + self.w[2] * drift
