...
def SaveCheckPoint(gen, dis, genOpt, disOpt, imgSize, epoch, folder):
    ...
    torch.save(checkpoint, os.path.join(folder, f"SN512Checkpoint_Size{imgSize}_Epoch{epoch}.pth"))
    


def LoadCheckPoint(imgSize, epoch, folder):
    return torch.load(os.path.join(folder, f"SN512Checkpoint_Size{imgSize}_Epoch{epoch}.pth"))
